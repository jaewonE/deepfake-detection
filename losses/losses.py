...
def f_cstency(cstency_hm_preds, cstency_hm_gt):
    ...
    cstency_matrix_ = torch.matmul(
        cstency_hm_preds.view(
            b_size, cst_hm_dim, -1)[np.arange(b_size), :, indices_].view(b_size, 1, cst_hm_dim),
        cstency_hm_preds.view(b_size, cst_hm_dim, -1)
    )
    cstency_matrix_ = cstency_matrix_.view(b_size, cstency_hm_gt.size(
        1), cst_hm_h, cst_hm_w) / math.sqrt(cst_hm_dim)
...
def _neg_pos_loss(hm_pred, hm_gt):
    ...
    neg_pos_gt, neg_pos_pred = torch.zeros(batch_size, 1, dtype=torch.float32).cuda(), \
        torch.zeros(batch_size, 1, dtype=torch.float32).cuda()
    hm_pred_ = torch.squeeze(torch.clone(hm_pred))

    for i in range(batch_size):
        neg_pos_gt[i] = torch.sum(
            hm_gt[i][pos_idxes[i, :, :]]) - torch.sum(hm_gt[i][neg_idxes[i, :, :]])
        neg_pos_pred[i] = torch.sum(
            hm_pred_[i][pos_idxes[i, :, :]]) - torch.sum(hm_pred_[i][neg_idxes[i, :, :]])
...
def _neg_loss(pred, gt, epsilon=0.35, noise_distribution=0.2, alpha=0.25):
    ...
    pos_loss = (1 - epsilon) * torch.log(pred) * \
        torch.pow(1 - pred, 2) * pos_inds
    pos_loss_noise = epsilon * \
        torch.log(pred) * torch.pow(1 - pred, 2) * \
        noise_distribution * pos_inds
...
def _distance_hm_cls_loss(cos_sim_ins, hm_preds, hm_gts, label_preds, label_gts, alpha=0.25):
    ...
    for i in range(0, b_size // 2):
        for j in range(0, b_size // 2):
            pos_hm_loss += (1 / 2) * \
                (1 - cos_sim_ins(hm_preds[i], hm_preds[j]))
            neg_hm_loss += (1 / 2) * (1 -
                                      cos_sim_ins(hm_preds[i], hm_preds[j + b_size // 2]))

    cos_loss = pos_hm_loss / ((b_size // 2)**2) - \
        neg_hm_loss / ((b_size // 2)**2)
...
class BaseLoss(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.bce_critetion = nn.BCELoss(reduction=self.cfg.ce_reduction)
        # For Cross Entropy Loss in general
        self.ce_critetion = CrossEntropyLoss(reduction=self.cfg.ce_reduction)
...
class CombinedLoss(nn.Module):
    def __init__(self, use_target_weight, cls_lmda=0.2, dst_lmda=0.2, reduction='mean', dist_cal=True, cls_cal=True, **kwargs):
        ...
        self.criterion_hm = JointsMSELoss(
            use_target_weight=use_target_weight, reduction=reduction)
...
class CombinedPolyLoss(nn.Module):
    ...
    def forward(self, hm_outputs, hm_targets, cls_preds, cls_gts):
        ...
        for i in range(hm_h):
            for j in range(hm_w):
                ce = binary_cross_entropy(hm_outputs_[:, :, i, j], torch.unsqueeze(
                    hm_targets[:, i, j], -1), reduction='none')
                pt = hm_outputs_[:, :, i, j]
                pt = torch.squeeze(pt)
                pt = torch.where(hm_targets[:, i, j] > 0, pt, 1 - pt)
                poly_loss += (ce + self.epsilon *
                              (1.0 - torch.unsqueeze(pt, -1)))
