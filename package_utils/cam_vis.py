...
def main():
    ...
    argparser.add_argument(
        '--cfg', help='Specify config to load', required=True)
    argparser.add_argument(
        '--target_layer', '-t', help='Specify layer names to visualize CAM', required=True)
    argparser.add_argument("--method", '-m', type=str,
                           default="CAM", help="CAM method to use")
    argparser.add_argument(
        '--image', '-i', help='Specify image to overlay CAM', required=True)
    argparser.add_argument("--savefig", type=str,
                           default=None, help="Path to save figure")
    argparser.add_argument("--rows", type=int, default=1,
                           help="Number of rows for the layout")
    argparser.add_argument("--class-idx", type=int,
                           default=0, help="Index of the class to inspect")
    argparser.add_argument("--alpha", type=float, default=0.5,
                           help="Transparency of the heatmap")
    argparser.add_argument('--cuda', action='store_true',
                           help='Running CAM with cuda')
    argparser.add_argument(
    ...
    assert os.path.exists(
        args.image), 'Image path must be valid, please check the path again!'
    ...
    img_tensor = transform(img_resize).to(torch.float32)
    if args.cuda:
    ...
    for idx, extractor in zip(range(1, len(cam_extractors) + 1), cam_extractors):
        ...
        activation_map = extractor(class_idx, scores)[0].to(
            torch.float32).squeeze(0).cpu()
    ...
    plt.tight_layout()
    if args.savefig:
        plt.savefig(args.savefig, dpi=200, transparent=True,
